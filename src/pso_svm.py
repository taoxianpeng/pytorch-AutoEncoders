...
def fitness_function(position,data):
    ...
    return confusion_matrix(y_train,y_train_pred)[0][1] + \
        confusion_matrix(y_train,y_train_pred)[1][0], \
            confusion_matrix(y_test,y_test_pred)[0][1] + \
                confusion_matrix(y_test,y_test_pred)[1][0]
...
def pso_svm(data):
    ...
    while iteration < args.n_iterations:
        ...
        print('最好的位置 : ',gbest_position)
        iteration = iteration + 1
    #返回svc训练好的模型,和函数,gamma值,以及c的值    
